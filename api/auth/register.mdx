...
<CodeGroup>
```bash cURL
curl -X POST https://api.appcourier.com/auth/clienteRegister \
...
<CodeGroup>
```bash cURL
curl -X POST https://api.appcourier.com/auth/clienteValidarCodigoRegistro \
  -H "Content-Type: application/json" \
  -d '{
    "email": "juan.perez@example.com",
    "codigo": "123456"
...
<CodeGroup>
```bash cURL
curl -X POST https://api.appcourier.com/auth/clienteCrear \
  -H "Content-Type: application/json" \
  -d '{
