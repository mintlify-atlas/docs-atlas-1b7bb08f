...
<CodeGroup>
```bash cURL
curl -X POST https://api.appcourier.com/auth/login \
...
<CodeGroup>
```bash cURL
curl -X POST https://api.appcourier.com/auth/loginCliente \
  -H "Content-Type: application/json" \
  -d '{
...
```bash
curl -X GET https://api.appcourier.com/encomienda/getEncomiendas \
